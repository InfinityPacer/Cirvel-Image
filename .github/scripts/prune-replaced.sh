...
prune_ghcr() {
  ...
  versions="$(gh api --paginate "/users/${GITHUB_REPOSITORY_OWNER}/packages/container/${package}/versions" \
  ...
  for digest in "${stale[@]}"; do
    ...
    gh api --silent --method DELETE "/users/${GITHUB_REPOSITORY_OWNER}/packages/container/${package}/versions/${id}"
